...
The first thing to do was to explore the state of the art regarding the modeling of energy communities. After exploring some references, we were able to reflect more on what we would do for this project. Indeed, we found that we lack understanding of the fairness problematics in the work we explored. That is why we decided to focus on this aspect in this project. Then I thought of a methodology that was validated with my supervisor, before starting to develop the code. The code was developed in python, using the Pyomo library for optimization. Finally, we were able to run some simulations and visualize the results. The work planning is shown in figure \ref{fig:planning}.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth, trim={0 19cm 0 4cm}, clip]{gantt.drawio.pdf}
