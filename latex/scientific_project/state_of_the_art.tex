...
A total cost can then be defined considering the power consumed $P_{cons}$ and sold $P_{sell}$.
\begin{equation}
    Cost = \sum_{t=T_0}^{T_{end}} p_{buy}(t)P_{cons}(t) - p_{sell}(t)P_{sell}(t)
\end{equation}
...
This method uses the feed-in tariff, which guarantees energy producers a fixed price for renewable electricity fed into the grid under long-term contracts. An important property is to have a fixed reference for the selling price in this method, that is why we consider a feed-in tariff.
...
Looking back at the different distribution methods presented previously, the rule based methods such as pricing mechanisms focus on meritocracy. While the redistribution methods can choose more to focus on the aspect they want to prioritize. Then the different indexes presented in \cite{article_1, these_new, article_8, article_2} can be used in order to check the fairness of the final distribution. 
...
Taking this into account, the optimization process can be adapted using different metrics as objectives. In \cite{these_sorel}, they consider for example self-consumption rate, CO\textsubscript{2} emissions and cost as metrics to optimize.